        ...
        {data.roles.map(r => (
          <div key={r.id} className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <span className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded mt-1 ${
              r.type === 'major' ? 'bg-gold-pale text-ink-secondary' : 'bg-moss-pale text-moss'
            }`}>
              {r.type}
            </span>
            ...
            {r.status === 'vacant' && (
              <div className="inline-block mt-2 px-2 py-0.5 rounded text-xs font-semibold bg-ember-pale text-ember">
