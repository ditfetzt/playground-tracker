      ...
      {data.tickets.length === 0 ? (
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-2.5">
          {data.tickets.map(t => {
            ...
            return (
              <div key={t.id} className="bg-white rounded-lg p-4 shadow-sm flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-sm">{p?.name || 'Unknown'} {p?.emoji || ''}</div>
                  <div className="text-xs text-ink-secondary">Source: {t.source}{t.notes ? ` — ${t.notes}` : ''}</div>
                </div>
                <span className={`text-xs font-semibold px-2.5 py-1 rounded ${
                  t.status === 'confirmed' ? 'bg-moss-pale text-moss' :
                  t.status === 'issued' ? 'bg-wood text-ink-secondary' :
                  'bg-gold-pale text-ink-secondary'
                }`}>
                  {t.status}
                </span>
              </div>
    ...
    <Modal open={open} onClose={onClose} title="Add Ticket">
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs font-semibold text-ink-secondary block mb-1">Source</label>
            <select value={form.source} onChange={set('source')} className="w-full px-3 py-2 border border-wood rounded-lg bg-surface text-sm outline-none focus:border-moss">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input value={form.notes} onChange={set('notes')} placeholder="Optional" className="w-full px-3 py-2 border border-wood rounded-lg bg-surface text-sm outline-none focus:border-moss" />
          </div>
        </div>
