    ...
    <div>
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-2.5">
        {data.profiles.map(p => {
          ...
          return (
            <div key={p.id} className="bg-card glass-card rounded-lg p-4 shadow-glow-purple/10 flex items-center gap-3">
              <span className="text-2xl">{p.emoji || '🙋'}</span>
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-sm">{p.name}</div>
                <div className="text-xs text-text-secondary">{hours.toFixed(1)}h / {target}h</div>
              </div>
    ...
    <Modal open={open} onClose={onClose} title="Log Volunteer Hours">
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs font-semibold text-text-secondary block mb-1">Hours</label>
            <input value={form.hours} onChange={set('hours')} type="number" step="0.5" min="0.5" required className="w-full px-3 py-2 border border-border-glow rounded-lg bg-surface/50 text-sm outline-none focus:border-moss" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
          ...
          <div>
            ...
            <input value={form.task} onChange={set('task')} placeholder="What did they do?" className="w-full px-3 py-2 border border-border-glow rounded-lg bg-surface/50 text-sm outline-none focus:border-moss" />
          </div>
        </div>
