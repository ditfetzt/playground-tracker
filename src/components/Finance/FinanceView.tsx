...
export function FinanceView() {
  ...
  const budgetClass = pct > 85 ? 'bg-danger' : pct > 65 ? 'bg-gold' : 'bg-moss'
    ...
    <div>
      ...
      <div className="grid grid-cols-3 max-md:grid-cols-1 gap-3 mb-4">
        <div className="bg-white rounded-lg p-4 shadow-sm">
          <div className="text-xs text-ink-secondary font-medium">Total Income</div>
          <div className="text-2xl font-bold text-moss mt-1">${income.toFixed(2)}</div>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-sm">
          <div className="text-xs text-ink-secondary font-medium">Total Expenses</div>
          <div className="text-2xl font-bold text-danger mt-1">${expense.toFixed(2)}</div>
        </div>
        <div className="bg-white rounded-lg p-4 shadow-sm">
          <div className="text-xs text-ink-secondary font-medium">Balance</div>
      ...
      <div className="h-3 bg-wood rounded-full overflow-hidden mb-4">
      ...
      {data.finances.length === 0 ? (
        ...
        <div className="overflow-x-auto bg-white rounded-lg shadow-sm">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs font-semibold text-ink-secondary uppercase tracking-wide">
                <th className="text-left px-4 py-2.5 bg-surface">Date</th>
                <th className="text-left px-4 py-2.5 bg-surface">Type</th>
                <th className="text-left px-4 py-2.5 bg-surface">Category</th>
                <th className="text-left px-4 py-2.5 bg-surface">Description</th>
                <th className="text-left px-4 py-2.5 bg-surface">Person</th>
                <th className="text-right px-4 py-2.5 bg-surface">Amount</th>
                <th className="text-left px-4 py-2.5 bg-surface">Status</th>
                {isAdmin && <th className="px-2 py-2.5 bg-surface"></th>}
              </tr>
            </thead>
            <tbody>
              {data.finances.map(f => (
                <tr key={f.id} className="hover:bg-surface transition-colors">
                  <td className="px-4 py-2 text-ink-secondary">{f.date || '-'}</td>
                  <td className="px-4 py-2">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded ${f.type === 'income' ? 'bg-moss-pale text-moss' : 'bg-ember-pale text-ember'}`}>
                  ...
                  <td className={`px-4 py-2 text-right font-mono font-medium ${f.type === 'income' ? 'text-moss' : 'text-danger'}`}>
                  ...
                  {isAdmin && (
                    <td className="px-2 py-2">
                      <button onClick={() => { if (confirm('Delete this entry?')) deleteFinance(f.id) }} className="p-1 rounded hover:bg-ember-pale text-ink-muted">
                        <Trash2 size={12} />
                      </button>
                    </td>
                  )}
  ...
  return (
    <Modal open={open} onClose={onClose} title="New Finance Entry">
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-semibold text-ink-secondary block mb-1">Amount ($)</label>
            <input value={form.amount} onChange={set('amount')} type="number" step="0.01" min="0" required className="w-full px-3 py-2 border border-wood rounded-lg bg-surface text-sm outline-none focus:border-moss" />
          </div>
          <div>
            <label className="text-xs font-semibold text-ink-secondary block mb-1">Person</label>
            <input value={form.person} onChange={set('person')} placeholder="Who?" className="w-full px-3 py-2 border border-wood rounded-lg bg-surface text-sm outline-none focus:border-moss" />
          </div>
        </div>
        <div>
          <label className="text-xs font-semibold text-ink-secondary block mb-1">Description</label>
          <input value={form.description} onChange={set('description')} required placeholder="What for?" className="w-full px-3 py-2 border border-wood rounded-lg bg-surface text-sm outline-none focus:border-moss" />
        </div>
        <div>
          <label className="text-xs font-semibold text-ink-secondary block mb-1">Status</label>
          <select value={form.status} onChange={set('status')} className="w-full px-3 py-2 border border-wood rounded-lg bg-surface text-sm outline-none focus:border-moss">
